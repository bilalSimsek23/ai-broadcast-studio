...
/**
 * Seeds the known "Gerçeğin Peşinde" demo content: the Show, the Hikmet AI
 * persona, Episode 1 with its presenter brief / AI brief / line-up / 6
 * discussion topics and their questions.
 *
 * Idempotent — every row is upserted on a stable natural key (show slug,
 * persona name, (show, episode_number), persona id in the line-up, topic
 * title, question text), so running it twice changes nothing and creates
 * nothing new. It uses the real models, enums and readiness services and
 * reaches "Ready" only through {@see MakeEpisodeReady}, never a direct write.
 * `users` / auth / sessions / jobs / cache / secrets are never touched.
 *
 * `--dry-run` performs the whole thing inside a rolled-back transaction and
 * reports what it would do, writing nothing.
 */
final class DemoSeedGerceginPesinde extends Command
{
    protected $signature = 'demo:seed-gercegin-pesinde
        {--dry-run : Run everything in a rolled-back transaction and report, without writing}';

    protected $description = 'Seed the "Gerçeğin Peşinde" show, the Hikmet persona and Episode 1 (briefs, line-up, topics, questions). Idempotent.';
    ...
    public function handle(): int
    {
        ...
        if ($dryRun) {
            DB::beginTransaction();
            try {
                $summary = $this->seed(dryRun: true);
            } finally {
                DB::rollBack();
            }
        } else {
            $summary = $this->seed(dryRun: false);
        }

        $this->renderReadiness($summary['readiness']);
    ...
    /**
     * @return array{dry_run: bool, show: Show, persona: AiPersona, episode: Episode, topics: int, questions: int, readiness: EpisodeReadiness}
     */
    private function seed(bool $dryRun): array
    {
        $show = $this->seedShow();
        ...
        $this->seedTopicsAndQuestions($episode);

        $episode->refresh();

        $readiness = app(AssessEpisodeReadiness::class)($episode);

        if ($readiness->isReady() && ! $dryRun) {
            app(MakeEpisodeReady::class)($episode);
            $episode->refresh();
        }

        $questions = EpisodeQuestion::query()
    ...
    private function seedTopicsAndQuestions(Episode $episode): void
    {
    ...
    private function renderReadiness(EpisodeReadiness $readiness): void
    {
        $rows = [];
        ...
        $this->newLine();
        $this->line('<info>Readiness assessment (AssessEpisodeReadiness)</info>');
        $this->table(['check', 'result', 'label'], $rows);
        $this->line(
            $readiness->isReady()
    ...
    /**
     * @param  array{dry_run: bool, show: Show, persona: AiPersona, episode: Episode, topics: int, questions: int, readiness: EpisodeReadiness}  $summary
     */
    private function renderSummary(array $summary): void
    {
        ...
        $this->newLine();
        $this->line($summary['dry_run']
            ? '<comment>DRY RUN — rolled back; nothing was written.</comment>'
            : '<info>demo:seed-gercegin-pesinde — done.</info>');

        $this->table(['field', 'value'], [
            ['Show', $summary['show']->uuid.'  —  '.$summary['show']->name.'  ('.$summary['show']->status->value.')'],
            ['AiPersona', $summary['persona']->uuid.'  —  '.$summary['persona']->name.'  ('.$summary['persona']->status->value.')'],
            ['Episode', $episode->uuid.'  —  '.$episode->title],
            ['Topics', (string) $summary['topics']],
            ['Questions', (string) $summary['questions']],
            ['Readiness', $summary['readiness']->isReady() ? 'ready (all blocking checks passed)' : 'NOT ready'],

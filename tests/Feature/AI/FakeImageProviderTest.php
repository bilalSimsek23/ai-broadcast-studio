...
class FakeImageProviderTest extends TestCase
{
    ...
    public function test_it_returns_a_decodable_png_and_records_the_call(): void
    {
        ...
        $this->assertNotFalse(base64_decode($image->base64, true));
        $this->assertStringStartsWith('data:image/png;base64,', $image->toDataUri());
        $this->assertSame($request, $provider->lastCall());
        $this->assertSame(1, $provider->callCount());
    }

    public function test_it_is_deterministic_across_instances(): void
    {
        $a = (new FakeImageProvider)->generate(new ImageGenerationRequest('x', '1024x1024'));
        $b = (new FakeImageProvider)->generate(new ImageGenerationRequest('y', '1536x1024'));
